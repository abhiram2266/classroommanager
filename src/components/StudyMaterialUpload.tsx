...
export const StudyMaterialUpload: React.FC<StudyMaterialUploadProps> = ({ onUploadSuccess }) => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validate file size (50MB max for study materials)
    ...
    try {
      ...
      await uploadStudyMaterial(
        ...
        formData.subject,
        formData.semester,
        'Anonymous Student', // In production, use actual user name from auth
    ...
    <div className="bg-gray-900 p-6 rounded-lg border border-green-600/30">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-lg font-semibold text-white" style={{ fontFamily: "'Audiowide', sans-serif" }}>
          Upload Study Material (Shared with Community)
        </h3>
      </div>
      <p className="text-xs text-green-400 mb-4">Note: All materials uploaded here will be visible to everyone in the college</p>
      ...
      {/* File Upload Area */}
      <div
        className="border-2 border-dashed border-yellow-600/50 rounded-lg p-8 text-center hover:border-yellow-600 transition-colors cursor-pointer mb-4"
